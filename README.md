...
- Home Route
  ...
  - Users should be able to see the sort by icon (`BsFilterLeft` from `react-icons/bs`) as shown in the Figma.
  - The default value for the Sort By filter should be `Lowest` (You can use the `sortByOptions` object from the path `src/App.jsx`).
...
- Use `react-loader-spinner` package for loading indicators. Use `Oval` loader with `color="gold"`, `height={40}`, `width={50}`
...
- **Note:**
  ...
  - Use **functional components** with **React hooks** (`useState`, `useEffect`, `useParams`, `useNavigate`) for all components.
  - Use `Oval` loader from `react-loader-spinner` package with `color="gold"`, `height={40}`, `width={50}`
...
- Footer:
  ...
  - The Footer component should consist of all social icons from the `react-icons/fa` third-party library.
  - The image in the Footer Component should contain alt text as `website-footer-logo`.
  - The Pintrest social icon (`FaPinterestSquare` from `react-icons/fa`) should contain the test id value as `pintrest-social-icon`.
  - The Instagram social icon (`FaInstagram` from `react-icons/fa`) should contain the test id value as `instagram-social-icon`.
  - The Twitter social icon (`FaTwitter` from `react-icons/fa`) should contain the test id value as `twitter-social-icon`.
  - The Facebook social icon (`FaFacebookSquare` from `react-icons/fa`) should contain the test id value as `facebook-social-icon`.
- Restaurant Details:
...
- Cart:
  ...
  - Use `HiOutlineMinusSm` from `react-icons/hi` for the decrement button icon.
  - Use `BsPlus` from `react-icons/bs` for the increment button icon.
  - Use `FaRupeeSign` from `react-icons/fa` for the currency symbol.
  - Use `FaCheckCircle` from `react-icons/fa` for the payment success checkmark icon.
